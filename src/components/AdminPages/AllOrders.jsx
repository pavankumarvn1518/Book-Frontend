...
const AllOrders = () => {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        const res = await axios.get(
          "https://bookbackend-duko.onrender.com/api/v1/get-all-orders",
          { headers }
        );
    ...
    fetch();
  }, []); // Dependency array updated to avoid infinite loop
  ...
  const submitChanges = async (i) => {
    if (!OrderHistory[i]) return;
    const id = OrderHistory[i]._id;
    try {
      const response = await axios.put(
        `https://bookbackend-duko.onrender.com/api/v1/update-status/${id}`,
        Values,
        { headers }
      );
